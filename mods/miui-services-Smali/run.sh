...
patch_A15_MaxFreeformCount() {

  local smali_mod2=$(find $workfile/miui-services/smali/com/android/server/wm/ -type f -iname "MiuiFreeFormStackDisplayStrategy.smali")
  ...
  local start_line_mod2=$(grep -n -m 1 ".method public getMaxMiuiFreeFormStackCount(Ljava/lang/String;Lcom/android/server/wm/MiuiFreeFormActivityStack;)I" $smali_mod2 | cut -d: -f1)
  # 从.start_line_mod1开始查找第一个.end method行号
  local end_line_mod2=$(tail -n +"$start_line_mod2" $smali_mod2 | grep -n -m 1 ".end method" | cut -d: -f1)
  # 计算.end method的行号
  local actual_end_line_mod2=$((start_line_mod2 + end_line_mod2 - 1))
  # 删除原方法
  sed -i "${start_line_mod2},${actual_end_line_mod2}d" $smali_mod2
  # 插入新方法
  sed -i "$((start_line_mod2 - 1))r $workfile/$android_target_version/add3.smali" $smali_mod2

  echo '修补调整小窗数量完成'

}

patch_MaxFreeFormCount() {
  ...
  local start_line_mod2=$(grep -n -m 1 ".method private getMaxMiuiFreeFormStackCount(Ljava/lang/String;Lcom/android/server/wm/MiuiFreeFormActivityStack;)I" $smali_mod2 | cut -d: -f1)
  echo "Debug: start_line_mod2 = $start_line_mod2"
  # 从.start_line_mod1开始查找第一个.end method行号
  local end_line_mod2=$(tail -n +"$start_line_mod2" $smali_mod2 | grep -n -m 1 ".end method" | cut -d: -f1)
  # 计算.end method的行号
  local actual_end_line_mod2=$((start_line_mod2 + end_line_mod2 - 1))
...
patch_MaxFreeFormCountEntry() {
  if [ "$android_target_version" -ge 16 ]; then
    patch_MaxFreeFormCount
  elif [ "$android_target_version" -le 15 ]; then
    patch_A15_MaxFreeformCount
  else
    echo "❌ Unsupported Android version for max free form count patch: $android_target_version"
    exit 1
  fi
}
